...
| 2 | Currency — the registry's true latest, publish times vs. PR open time, and changelogs across the gap. Whether a destructive fix in that gap can fire *here* is settled by running the rule three ways — as configured, named over an input where it **must** fire, and named over nothing — because a lint run that printed nothing and one that never ran are the same output |
...
| 7 | Report — and the verdict *derived* from the evidence rather than judged: a table mapping findings to one of the three recommendations, a precedence for when phases disagree (they are meant to), and confidence as a function of what could not be derived. `scripts/verify_run.py` reads the record a `PreToolUse` hook keeps of every command this audit issued and every file it read, so the deviation list is computed rather than recalled — the report used to assert *"I followed this procedure"* by silence, and twice that assertion was false. Also where the worktrees and branch get cleaned up, because it is the only phase every audit reaches |
